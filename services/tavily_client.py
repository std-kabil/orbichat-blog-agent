...
class TavilySearchClient:
    def __init__(self, settings: Settings, http_client: httpx.AsyncClient | None = None) -> None:
        if not settings.tavily_api_key:
            raise ServiceConfigurationError("TAVILY_API_KEY is required for Tavily search")
    ...
    async def search(
        ...
        *,
        ...
        try:
            ...
            payload = require_mapping(response.json(), SearchProvider.TAVILY.value)
    ...
    async def _post(self, *, query: str, max_results: int) -> httpx.Response:
        ...
        if self._http_client is not None:
            response = await self._http_client.post(TAVILY_SEARCH_URL, json=body, timeout=30)
            raise_for_status(response)
            return response

        async with httpx.AsyncClient() as client:
            response = await client.post(TAVILY_SEARCH_URL, json=body, timeout=30)
            raise_for_status(response)
            return response


def _normalize_tavily_results(payload: dict[str, object]) -> list[NormalizedSearchResult]:
